...
import {
  PagedItemView,
  PagedItemPresenter,
} from "../../model.presenter/PagedItemPresenter";

interface Props<T> {
  featureUrl: string;
  presenterFactory: (view: PagedItemView<T>) => PagedItemPresenter<T, any>;
  itemComponent: React.ComponentType<{ item: T; featurePath: string }>;
}

const ItemScroller = <T,>({
  featureUrl,
  presenterFactory,
    ...
    <div className="container px-0 overflow-visible vh-100">
      ...
      >
        {items.map((item, index) => (
          ...
          >
            <ItemComponent item={item} featurePath={featureUrl} />
          </div>
        ))}
      </InfiniteScroll>
    </div>
